<template>
    <header class="flex items-center justify-between h-16 px-4 text-text bg-card border-b border-gray-200">
        ...
        <div class="flex items-center space-x-4">
            <button  @click="toggleTheme" class="p-2 text-gray-500 rounded-full  hover:bg-gray-100" title="Notifications">
                {{ isDark ? '☀️' : '🌙' }}
            </button>
            <router-link 
                to="/" 
...
<script>
    export default {
        data() {
            return { isDark: false, searchTerm: '' }
        },
        mounted() {
